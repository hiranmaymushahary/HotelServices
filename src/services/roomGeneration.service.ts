...
export async function processDateBatch(roomCategory: RoomCategory, startDate: Date, endDate: Date, priceOverride?: number) {
    ...
    const currentDate = new Date(startDate);
   // SELECT * FROM ROOM_CATEGORY WHERE ID = ? AND DATE_OF_AVAILABILITY BETWEEN ? and ?  or we can do in while loop also but we should not do it can overload our server if we 
   // do while loop
    while(currentDate <= endDate) {
